...
function disableWhosWatching(value) {
  const LeanbackRecurringActions = JSON.parse(
    localStorage["yt.leanback.default::recurring_actions"]
  );
  const date = new Date();
  if (!value) {
    // Setting it after 7 days should be enough, as it'll get executed every time the app launches.
    date.setDate(date.getDate() + 7);
    LeanbackRecurringActions.data.data[
      "startup-screen-account-selector-with-guest"
    ] &&
      (LeanbackRecurringActions.data.data[
        "startup-screen-account-selector-with-guest"
      ].lastFired = date.getTime());
    LeanbackRecurringActions.data.data.whos_watching_fullscreen_zero_accounts.lastFired =
      date.getTime();
    LeanbackRecurringActions.data.data[
    ...
    localStorage["yt.leanback.default::recurring_actions"] = JSON.stringify(
      LeanbackRecurringActions
    );
  } else {
